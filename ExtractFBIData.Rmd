...
```{r test, results ='asis', message = FALSE, warning = FALSE, echo = FALSE}
xt <- capture.output(print(xtable(prettyNum(comboBottomAndTop,
                       big.mark=",",
                       preserve.width="none"),
             digits=0, 
...
xt_mod <- gsub("(\\s|^)(-\\S*)", "\\1\\\\textcolor{red}{\\2}", xt)
cat(xt_mod, sep="\n")
...
dateStartAb109 <- as.Date("2011-10-01")
...
ggplot(data = comboJailProfSets, aes(x = YearMonth, y = count ))  +
  ...
  geom_text(data = ann_text,label = "AB 109 starts",hjust=0, vjust=1.50, angle = 90, size = 4, colour = "blue") +
