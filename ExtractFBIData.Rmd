---
title: "Property and violent crime rate chage between 2009 and 2013"
output:
  pdf_document:
    fig_width: 7
    fig_height: 6
...
This analysis compares the property and violent crime rate changes in Los Angeles County by city utilizing data from the Federal Bureau of Investigation's (FBI) Uniform Crime Report (UCR). These data can be found in Table 8 for years 2009 and 2013 [here.](http://www.fbi.gov/about-us/cjis/ucr/ucr)  

In addition, inspiration for this report came from the 'Realignment, Incarceration, and Crime Trends in California
' a publication completed by the Public Policy Institute of California that shows the impact of AB109. This report can be found [here.](http://www.ppic.org/main/publication_quick.asp?i=1151)  

For this analysis, the crime rates fo two years before and two years after the [AB-109 Criminal justice alignment](http://leginfo.legislature.ca.gov/faces/billNavClient.xhtml?bill_id=201120120AB109) started were selected to determine rate changes.
...
citiesInLACounty <- read.dbf("I:/maps/scagCityBoundaries/city_boundary_scag_2010LACountyOnly.dbf", as.is = FALSE)
...
For example, if a city has 10 robberies in a given year and 50,000 residents on the same year then the robbery rate per 100,000 residents is:    
...
print(xtable(prettyNum(comboCityStats,big.mark=","),
             ...
             in Los Angeles county for property rates in 2009 and 2009",  
             label="tblCity", 
             # align = "rl{5cm}r{3cm}r{3cm}r{3cm}r{3cm}"
             ), 
      caption.placement = "bottom", 
...
ggplot(citiesInLACountyCombo20092013, aes(colour=factor(IncreasedProperty))) + 
  ...
  annotate("text", label = paste ("Median property crime rate for L.A. County cities in 2013 was", 
           ...
           hjust=0, vjust=-1.0, angle = 90, size = 4, colour = "black") + 
  annotate("text", label = 
...
For this analysis data from The Board of State and Community Corrections. Jail Survey Data available is between January 2001 and September 2014. These data can be accessed [here](https://app.bscc.ca.gov/joq//jps/QuerySelection.asp). It is important to note that Prop 47 passed in November 2014, and any effects it had in jail population counts can't be analyzed at this time.     
...
In conclusion and only for the variables analyzed, the number of highest count by month and inmates receiving psych medication increased after AB 109 started. However the other variables analyzed had decreasing trends or the trends did not changed after AB109 began.
...
# add a variable to identify each
...
```{rplot13, fig.width= 10, fig.height = 15, echo=FALSE, fig.cap= "Selected Jail Survey variables for Los Angeles County from January 2001 to December 2014", warning=FALSE}
